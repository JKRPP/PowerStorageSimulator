...
class StoragePowerPlant:
    def __init__(self, power = 10.0, capacity = 15.0, efficiency=0.9, upper_limit=0.9, lower_limit=0.1, degradation=5.0):
        self.power = power
    ...
    def _calculate_charge(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data.loc[charge_ts, "power_cost"] = data.loc[charge_ts, "price_eur_mwh"]*data.loc[charge_ts, "Loading"]/4
        data.loc[discharge_ts, "power_revenue"] = -1*self.efficiency*data.loc[discharge_ts, "price_eur_mwh"]*data.loc[discharge_ts, "Loading"]/4

        data["degradation_cost"] = self.degradation*data["Loading"].abs()/4
        data["total_profit"] = data["power_revenue"] - (data["degradation_cost"] + data["power_cost"])
