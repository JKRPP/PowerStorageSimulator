...
def residual_load_bars(mix_data:pd.DataFrame, result_data:pd.DataFrame):
    ...
    residual_charge = mix_data[mix_data.index.isin(charge_indices)]["Residual load"].mean()
    residual_discharge = mix_data[mix_data.index.isin(discharge_indices)]["Residual load"].mean()

    renewable_charge = mix_data[mix_data.index.isin(charge_indices)]["Renewable share of load"].mean()
    renewable_discharge = mix_data[mix_data.index.isin(discharge_indices)]["Renewable share of load"].mean()
    ...
    percent_change_residual = ((residual_discharge-residual_charge)*100/residual_discharge).__round__(2)
    percent_change_renewables = ((renewable_charge-renewable_discharge)*100/renewable_charge).__round__(2)

    col1, col2 = st.columns(2)
    with col1:
        st.bar_chart(result_df, x="Operation", y="Residuallast", color="Yellow", sort=False)
        st.metric("Residuallast während der Entladung", str(residual_discharge.__round__(2)) + " MWh",str(percent_change_residual) + " %")
    with col2:
        st.bar_chart(result_df, x="Operation", y="Anteil Erneuerbare", color="Green", sort=False)
        st.metric("Anteil erneuerbarer während der Ladung", str(renewable_charge.__round__(2)) + " %",str(percent_change_renewables) + " %")
...
def plant_selector():
    ...
    """
    plant_types = {
        "Klein (Li-ion)": {"power": 10.0, "capacity" : 15.0, "efficiency":0.9, "upper_limit":0.9, "lower_limit":0.1, "degradation":10.0},
        "Groß (Li-ion)": {"power": 50.0, "capacity" : 150.0, "efficiency":0.9, "upper_limit":0.9, "lower_limit":0.1, "degradation":7.5},
        "Redox-Flow": {"power": 20.0, "capacity" : 120.0, "efficiency":0.7, "upper_limit":0.9, "lower_limit":0.1, "degradation":4.0},
        "Pumpspeicher": {"power": 300.0, "capacity" : 1800.0, "efficiency":0.8, "upper_limit":0.9, "lower_limit":0.1, "degradation":3.0},
        "Elektroauto": {"power": 0.008, "capacity" : 0.095, "efficiency":0.9, "upper_limit":0.9, "lower_limit":0.5, "degradation":20.0},
    }

    selected = st.selectbox("Simulierten Netzspeichertyp auswählen:", plant_types.keys())
